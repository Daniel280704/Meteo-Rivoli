...
def interpella_groq(dati_testuali, oggi_str, domani_str):
    ...
    prompt = f"""
    Sei un meteorologo professionista. Il tuo compito è scrivere un bollettino discorsivo, fluido ed elegante per Rivoli (TO) QUOTIDIANO (Oggi e Domani).
...
def main():
    ...
    dt_fine_estrazione = dt_oggi + timedelta(days=2) 
    usa_seamless = False
    ...
    indici_validi = [i for i, t in enumerate(orari) if 0 <= (datetime.fromisoformat(t).date() - dt_oggi.date()).days <= 1]
    ...
    testo_per_ia = ""
    for g, nome_giorno in zip([0, 1], [oggi_str, domani_str]):
        dg = dati_giorni[g]
        testo_per_ia += f"GIORNO: {nome_giorno}\n"
